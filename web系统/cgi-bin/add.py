...
if 'HTTP_COOKIE' in os.environ:
    ...
    try:
        ...
        if data == '5d4565b02217e1761d3b981c1f1c6878':
            ...
            sql = "INSERT INTO user(uname, upwd,wz,zb,types,remaining_days) VALUES ('%s', '%s','%s','%s',%d,%d)" % (uname, pwd, wz, zb, int(types), day)
            mycursor.execute(sql)
            mydb.commit()
            print('<h1>')
            print("1 条记录已插入, ID:", uname)
            print('</h1>')
        else:
            print(fail)
    except KeyError:
